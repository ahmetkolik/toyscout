...
> ⚠️ **Two folders, kept in sync (merged 2026-09-24).** `~/Downloads/Toyscout` is a real git clone of `github.com/ahmetkolik/toyscout` — pushes from here deploy to production. `~/Projects/toyscout` is the plain (non-git) folder the `launchd` agents, `bestseller_sync.py` and `verify.sh` target. They had diverged in both directions (Projects: 165 products + `new_releases_sync.py`, `gsc_theme_search.py`, newer `TASKS.md`; clone: the 1,016-product catalog). On 2026-09-24 the Projects-only work was merged into the clone and the clone was then rsynced over the code/catalog/assets/tasks of Projects (backup of everything overwritten: `~/Projects/toyscout-backup-20260924/`). **Rule: never let them diverge again** — make changes in one place and rsync (or `git pull`/push) to the other before running agents or deploying; never deploy `~/Projects/toyscout` blindly and never copy a stale `js/data.js` over the other. `tasks/TASKS.md` §A0/§B0-AGU7 says stray Downloads copies should be deleted; that rule is superseded by this setup until the user says otherwise (past incident: a forked Downloads copy silently diverged and 5 products never went live).
...
- Quality rules (same as `bestseller_sync.py`): rating ≥ 4.4, ≥ 50 reviews, price present, at least one image, title not ≥ 90 % similar to an existing product. Result of the 2026-09-24 run: 134 → 1,004 products (+12 more merged from `~/Projects/toyscout` = 1,016).
...
5. ~~`~/Projects/toyscout` stale/diverged~~ — merged and synced 2026-09-24 (see the warning at the top). Watch for re-divergence: the agents write to Projects, deploys go from the clone.
6. **Fixed 2026-09-25 (pre-render):** `products/prerender.py` generates `product/<cat>/<idx>/index.html` and `shop/<cat>/index.html` (unique title/description/canonical/og, Product/ItemList JSON-LD, readable summary in `<noscript>`); Vercel serves these files before the `/index.html` rewrites. **Re-run `python3 products/prerender.py` after ANY change to `js/data.js` or the `index.html` head/scripts (then `stamp_data_version.py`) and commit `product/` + `shop/`.** Old note: the SPA serves the same shell for every route and sets title/meta/canonical client-side (`updateSeo()`), so Google has to render JS to see per-product titles — expect slow indexing. (Reviews are no longer missing; see Data layer.)
